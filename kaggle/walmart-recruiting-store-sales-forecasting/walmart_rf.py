...
if not os.path.exists(MODEL_DIR): os.mkdir(MODEL_DIR)                               #해당 폴더가 있는지 확인하고 없으면 폴더를 만듦
...
for i in range(len(full_train_data)):
    if full_train_data[i,3] == 'TRUE': full_train_data[i,3] = 0
    else: full_train_data[i,3] = 1
for i in range(len(full_test_data)):
    if full_test_data[i,2] == 'TRUE': full_test_data[i,2] = 0
    else: full_test_data[i,2] = 1
...
# train 필요 열만 추출
tr_dept = full_train_data_2[:,1].reshape(-1,1) #1
tr_month = full_train_data_2[:,3].reshape(-1,1) #1
tr_week = full_train_data_2[:,5].reshape(-1,1) #1
tr_mark = full_train_data_2[:, 9:14] #5  
tr_size_sales = full_train_data_2[:, -2:] #2
...
print(full_train_data_2)
print(full_test_data_2)
print(Id)

'''
train_corr = full_train_data_2.astype(float)
# Store, Year, Day, IsHoliday, Temperature, Fuel_Size, CPI, Unemployment는 상관계수 너무 낮아서 걍 버림
...
##### RF TRAING #####
print('\nSTART RF\n')
rf = RandomForestRegressor(n_estimators = 50, max_depth = 23, max_features = 8, min_samples_split = 2, min_samples_leaf = 1, n_jobs = -1)    
